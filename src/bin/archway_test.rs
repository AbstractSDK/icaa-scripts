...
fn deploy() -> anyhow::Result<()> {
    ...
    let interchain = DaemonInterchainEnv::new(rt.handle(), vec![(JUNO_1, None), (ARCHWAY_1, None), (OSMOSIS_1, None)], &ChannelCreationValidator)?;
    // interchain.with_log();
    ...
    let mut home_ibc_client = IbcClient::new(IBC_CLIENT_ID, juno.clone());
    home_ibc_client.set_address(&ibc_client_address);
    let mut remote_proxies = home_ibc_client.list_remote_proxies_by_account_id(home_account_id.clone())?.proxies;
    ...
    // check for archway
    if remote_proxies.iter().find(|(chain, _)| chain == &ChainName::from_str(FIRST_HOP_CHAIN_NAME).unwrap()).is_none() {
        ...
        let remote_proxies = home_ibc_client.list_remote_proxies_by_account_id(home_account_id.clone())?.proxies;
        println!("accounts: {:?}", remote_proxies);
    } else {
    ...
    let archway_acc = AbstractAccount::new(&Abstract::load_from(archway)?, archway_account_id.clone());
    ...
    let ibc_client_address = archway_acc.manager.module_info(IBC_CLIENT_ID)?.unwrap().address;
    let mut archway_ibc_client = IbcClient::new(IBC_CLIENT_ID, interchain.chain(ARCHWAY_1.chain_id)?);
    archway_ibc_client.set_address(&ibc_client_address);
    ...
    if remote_proxies.iter().find(|(chain, _)| chain == &ChainName::from_str(SECOND_HOP_CHAIN_NAME).unwrap()).is_none() {
        ...
        let mut remote_proxies = archway_ibc_client.list_remote_proxies_by_account_id(archway_account_id.clone())?.proxies;
    ...
    let osmosis_account_id = AccountId::new(
        home_acc.id()?.seq(),
        AccountTrace::Remote(vec![ChainName::from_str(HOME_CHAIN_NAME)?, ChainName::from_str(FIRST_HOP_CHAIN_NAME)?]),
    )?;
    let osmosis_acc = AbstractAccount::new(&Abstract::load_from(osmosis)?, osmosis_account_id.clone());
    ...
    let ibc_client_address = osmosis_acc.manager.module_info(IBC_CLIENT_ID)?.unwrap().address;
    let mut osmosis_ibc_client = IbcClient::new(IBC_CLIENT_ID, interchain.chain(OSMOSIS_1.chain_id)?);
    osmosis_ibc_client.set_address(&ibc_client_address);
    let remote_proxies = osmosis_ibc_client.list_remote_proxies_by_account_id(osmosis_account_id.clone())?.proxies;
    println!("osmosis remote_proxies: {:?}", remote_proxies);


    // check whether juno>archway>osmosis has registered juno
    if remote_proxies.iter().find(|(chain, _)| chain == &ChainName::from_str(HOME_CHAIN_NAME).unwrap()).is_none() {
        ...
        let mut remote_proxies = osmosis_ibc_client.list_remote_proxies_by_account_id(osmosis_account_id.clone())?.proxies;
        println!("accounts: {:?}", remote_proxies);
    } else {
